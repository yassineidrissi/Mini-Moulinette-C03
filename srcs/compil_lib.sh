...
func_compil_lib()
{
	...
	printf "%.s${CHAR_LENGTH}" $(seq 1 ${TITLE_LENGTH})
	printf "\n\n${DEFAULT}"
	if [ -e "${PATH_LIBFT}"/c03.a ]
	then
		rm -f "${PATH_LIBFT}"/c03.a
	fi
	...
	then
		...
		then
			...
			then
				...
				if [ ! -e "${PATH_LIBFT}"/c03.a ] && [ ! -e "${PATH_LIBFT}"/ft_*.o ]
				then
		...
		printf "\n$> ls -la c03.a\n" >> "${PATH_DEEPTHOUGHT}"/deepthought
		ls -la "${PATH_LIBFT}"/c03.a >> "${PATH_DEEPTHOUGHT}"/deepthought
		if [ -e "${PATH_LIBFT}"/c03.a ]
		then

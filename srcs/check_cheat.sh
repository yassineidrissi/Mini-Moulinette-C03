...
check_cheating()
{
	if [ -e "${PATH_LIBFT}"/$1 ]
	then
		clang -Wextra -Wall -Werror "${PATH_LIBFT}"/$1 "${PATH_TEST}"/main_check_cheating.c "${PATH_LIBFT}"/c03.a
	else
		clang -Wextra -Wall -Werror "${PATH_LIBFT}"/$($1 | sed 's/_bonus//') "${PATH_TEST}"/main_check_cheating.c "${PATH_LIBFT}"/c03.a
	fi
